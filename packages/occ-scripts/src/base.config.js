...
// 扫描微应用入口规则
const scan_root = cfg.scan_root ? cfg.scan_root : 'occ-apps';
// 输出资源的文件夹
const dist_root = cfg.dist_root ? cfg.dist_root : 'occ-publish';
...
const config = {
    ...
    module: {
        rules: [{
            ...
            include: [path.resolve('.', 'occ-apps'), path.resolve('.', 'occ-common'), path.resolve('.', scan_root)],
            use: [{
    ...
    resolve: {
        ...
        alias: {
            'occ-apps': path.resolve('.', 'occ-apps/'),
            'occ-common': path.resolve('.', 'occ-common/src/'),
            components: path.resolve('.', 'occ-common/src/components/'),
            static: path.resolve('.', 'occ-common/src/static/'),
            utils: path.resolve('.', 'occ-common/src/utils/')
        }
    },
    plugins: [
        new webpack.BannerPlugin({
            banner: 'build:occ hash:[hash], chunkhash:[chunkhash], name:[name], filebase:[filebase], query:[query], file:[file]'
        }),
